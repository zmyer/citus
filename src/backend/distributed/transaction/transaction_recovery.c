...
static void RecoverPreparedTransactions(void);
static void RecoverWorkerTransactions(int groupId, WorkerNode *workerNode);
...
LogPreparedTransactions(List *connectionList)
{
	...
	foreach(connectionCell, connectionList)
	{
		...
		char transactionState = transactionConnection->transactionState;
		int groupId = transactionConnection->groupId;
		char *transactionName = transactionConnection->transactionName;

		Assert(transactionState == TRANSACTION_STATE_PREPARED);

		LogTransactionRecord(groupId, transactionName);
	}
}
...
RecoverPreparedTransactions(void)
{
	...
	foreach(workerNodeCell, workerList)
	{
		...
		RecoverWorkerTransactions(groupId++, workerNode);
	}
}
 ...
 */
static void
RecoverWorkerTransactions(int groupId, WorkerNode *workerNode)
{
	...
	List *confirmedTransactionList = NIL;
	ListCell *confirmedTransactionCell = NULL;
	...
	/*
	 * Transactions that have no pending prepared transaction are implictly
	 * confirmed. Find the set difference between unconfirmedTransactionList
	 * and pendingTransactionList.
	 */
	confirmedTransactionList = NameListDifference(unconfirmedTransactionList,
												  pendingTransactionList);

	/*
	 * For eahc pending prepared transaction, check whether there is a transaction
	...
	foreach(pendingTransactionCell, pendingTransactionList)
	{
		...
		else
		{
			...
			appendStringInfo(command, "ABORT PREPARED '%s'", transactionName);
		}
		...
		confirmedTransactionList = lappend(confirmedTransactionList,
										   transactionName);
	}

	/* we can remove the transaction records of confirmed transactions */
	foreach(confirmedTransactionCell, confirmedTransactionList)
	{
		char *transactionName = (char *) lfirst(confirmedTransactionCell);
...
{
	char *leftString = (char *) leftPointer;
	char *rightString = (char *) rightPointer;
				 ...
				 int *matchIndex)
{
	...
	while (searchIndex < nameCount)
	{
		...
		compareResult = CompareNames(needle, testName);
...
PendingWorkerTransactionList(PGconn* connection)
{
	...
	if (PQresultStatus(result) != PGRES_TUPLES_OK)
	{
		...
		ereport(WARNING, (errmsg("could not get prepared transactions "
								 "from %s:%s",
								 nodeName, nodePort)));
	...
	for (rowIndex = 0; rowIndex < rowCount; rowIndex++)
	{
		...
		transactionNames = lappend(transactionNames, transactionName);
	}
...
UnconfirmedWorkerTransactionsList(int groupId)
{
	...
	pgDistTransaction = heap_open(DistTransactionRelationId(), ExclusiveLock);
	...
	heap_close(pgDistTransaction, ExclusiveLock);

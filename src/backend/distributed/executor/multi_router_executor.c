 ...
 */
static void
ExecuteSingleShardSelect(QueryDesc *queryDesc, uint64 numberTuples, Task *task, EState *executorState,
						 TupleDesc tupleDescriptor, DestReceiver *destination)
{
	...
	if (routerState->tuplestorestate == NULL)
	{
		...
		resultsOK = ExecuteTaskAndStoreResults(task, tupleDescriptor,
	...
	for (;;)
	{
		bool nextTuple = tuplestore_gettupleslot(routerState->tuplestorestate,

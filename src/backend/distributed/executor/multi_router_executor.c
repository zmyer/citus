...
static bool ExecuteTaskAndStoreResults(QueryDesc *queryDesc,
									   ...
									   bool expectResults);
static int64 ReturnRowsFromTuplestore(uint64 tupleCount, EState *executorState,
...
RouterExecutorRun(QueryDesc *queryDesc, ScanDirection direction, long count, Task *task)
{
	...
	if (routerState->tuplestorestate != NULL)
	{
		...
		returnedRows = ReturnRowsFromTuplestore(count, estate, resultTupleDescriptor,
												destination,
 ...
 */
static int64
ReturnRowsFromTuplestore(uint64 tupleCount, EState *executorState,
 ...
 * rows.  Returns true if a non-errorr result has been returned, false if
	...
	*rows = 0;
	...
	while (true)
	{
		...
		if (status == PGRES_COMMAND_OK)
		{
			...
			*rows += pg_atoi(currentAffectedTupleString, sizeof(int32), 0);
		}
		else
		{

...
#endif
static ShardInterval * DetermineTargetShardInterval(Query *query, TaskType taskType);
...
{
	ShardInterval *shardInterval = DetermineTargetShardInterval(query, MODIFY_TASK);
...
/*
 * DetermineTargetShardInterval determines the single shard targeted by a
 * provided command. If no matching shards exist, or if the modification targets
 * more than one one shard, this function raises an error.
 */
static ShardInterval *
DetermineTargetShardInterval(Query *query, TaskType taskType)
{
	bool modifyTask = (taskType == MODIFY_TASK);
...
{
	Task *task = NULL;
	ShardInterval *shardInterval = DetermineTargetShardInterval(query, ROUTER_TASK);
	StringInfo queryString = makeStringInfo();
